...
namespace MXNetDotNet.Numerics
{
    ...
    {
        public static TOut Concatenate(uint axis = 0 ,params TOut[] inputList)
        {
            var ndim = inputList.First().Shape.Dimension;
            if (inputList.Any(a => a.Shape.Dimension != ndim))
            {
            ...
            var dstIndex = 0;
            for (uint inputIndex = 0; inputIndex < inputList.Length; inputIndex++)
            {
